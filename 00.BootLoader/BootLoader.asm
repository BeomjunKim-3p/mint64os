...
jmp 0x07C0:START          ; cs 세그먼트 레지스터에 0x07C0를 복사하면서 START 레이블로 점프

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; 환경설정값
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
TOTALSECTORCOUNT: dw 0    ; 부트로더를 제외한 OS 이미지의 크기
                          ; 최대 1152섹터(0x90000byte)까지 가능
                          ; 빌드 유틸리티에서 재설정되므로 0으로 임시 설정

KERNEL32SECTORCOUNT: dw 0 ; 보호 모드 커널의 총 섹터 수
...
START:
    ...
    ; 스택을 0x0000:0x0000 to 0x0000:000F 영역에 64KB 크기로 할당
    ...
    ; 화면을 모두 지우고, 속성값을 0x07(검정바탕에 흰색 문자)로 설정
    mov si, 0                      ; SI 레지스터(문자열 원본 인덱스 레지스터)를 초기화

.SCREENCLEARLOOP:                  ; 화면을 지우는 루프
    mov byte [es: si], 0           ; 비디오 메모리의 문자가 위치하는 주소에
                                   ; 0을 저장해서 문자를 삭제
    mov byte [es: si+1], TEXTCOLOR ; 비디오 메모리의 속성값이 위치하는 주소에
                                   ; 0x07(검정바탕에 흰색 문자)를 복사

    add si,2                       ; 다음 문자 위치로 이동

    cmp si, 80 * 25 * 2            ; 화면의 전체 크기는 80*25 문자이고,
                                   ; 출력한 문자의 수를 의미하는 SI 레지스터와 비교
    jl .SCREENCLEARLOOP            ; SI 레지스터가 80*25*2보다 작으면 루프 반복

    ; 화면 상단에 시작 메시지(BootLoader Starting...) 출력
    push TEXTCOLOR                 ; 일반 메시지의 속성값을 스택에 푸시
    push BOOTLOADERMESSAGE         ; 출력할 메시지(BOOTLOADERMESSAGE)의 주소를 스택에 푸시
    xor dx, dx
    mov dl, [SCREENLINE]
    push dx            ; 화면의 Y좌표를 스택에 푸시 (0)
    push 33                        ; 화면의 X좌표를 스택에 푸시
    call PRINTMESSAGE              ; PRINTMESSAGE 함수 호출
    add sp, 8                      ; 푸시한 3개의 인자(각 2바이트씩)를 스택에서 제거 (cdecl)

    add byte [SCREENLINE], 2        ; 화면 y좌표 변경(2)

    ; OS 이미지를 로딩한다는 메시지 출력
    push TEXTCOLOR                 ; 일반 메시지의 속성값을 스택에 푸시
    push IMAGELOADINGMESSAGE       ; 출력할 메시지(IMAGELOADINGMESSAGE)의 주소를 스택에 푸시
    xor dx, dx
    mov dl, [SCREENLINE]
    push dx              ; 화면의 Y좌표를 스택에 푸시 (0)
    push 1                         ; 화면의 X좌표를 스택에 푸시
    call PRINTMESSAGE              ; PRINTMESSAGE 함수 호출
    add sp, 8                      ; 푸시한 4개의 인자(각 2바이트씩)를 스택에서 제거 (cdecl)
...
RESETDISK:
    ...
    mov bx, 0x0000                         ; BX 레지스터에 0x0000을 설정하여 복사할 주소를 0x1000:0000으로 최종 설정
...
READDATA:                                  ; 디스크를 읽는 코드의 시작
    ...
    je READND                              ; 복사할 섹터 수가 0이라면 다 복사 했으므로 READND 레이블로 점프
...
; 디스크 에러를 처리하는 함수
HANDLEDISKERROR:
    push ERRORCOLOR             ; 에러 문자열의 색상 속성값을 스택에 푸시
    push ERRORMESSAGE       ; 에러 문자열의 주소를 스택에 푸시
    xor dx, dx
    mov dl, [SCREENLINE]
    push dx                      ; 화면의 Y좌표를 스택에 푸시
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; 부트로더 시작 메시지
BOOTLOADERMESSAGE: db 'OS BootLoader', 0      ; 출력할 메시지 정의
                                                        ; 마지막은 0으로 설정하여 .MESSAGELOOP에서 문자열이 종료되었음을 알 수 있도록 함

ERRORMESSAGE db "[Error]", 0 
IMAGELOADINGMESSAGE db "Loading OS Image From Disk", 0
