...
int main(int argc, char *argv[])
{
	...
	// 커맨드 라인 옵션 검사
	if (argc < 4) {
		fprintf(stderr, "[ERROR] ImageMaker.exe BootLoader.bin "
	...
	// 파일 크기를 섹터 크기인 512바이트로 맞추기 위해 나머지 부분을
	// 0x00으로 채움
                            ...
                            int iKernel32SectorCount)
{
	...
	if (lPosition == -1) {
		fprintf(stderr,
		        "[ERROR] lseek fail. Return value = %ld, errno = %d, "
		        "%d\n",
