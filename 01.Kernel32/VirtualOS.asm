...
jmp 0x1000:START                                                    ; CS 세그먼트 레지스터에 0x1000을 복사하면서 START 레이블로 점프
...
START:
    ...
    %rep TOTALSECTORCOUNT                                           ; TOTALSECTORCOUNT에 저장된 값만큼 아래 코드를 반복
        ...
        mov byte [es: si + (160*4) + 1], 0x0F                       ; 밝은 회색 글자색 설정
        ...
        %else                                                       ; 마지막 섹터가 아니라면                                   ; 섹터 수를 1 증가
            jmp (0x1000 + i * 0x20): 0x0000                         ; 다음 섹터 오프셋으로 이동
        %endif                                                      ; if문의 끝

        times (512 - ($ - $$) % 512) db 0x90                        ; $: 현재 라인의 주소
                                                                    ; $$: 현재 섹션(.text)의 시작 주소
                                                                    ; $ - $$: 현재 섹션을 기준으로 하는 오프셋
